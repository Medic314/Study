...
function Area:update(dt)
    ...
    if not paused then
        ...
        for i = #self.game_objects, 1, -1 do
            local game_object = self.game_objects[i]
            if game_object.layer == 'menu' or game_object.layer == 'menu2' then
                game_object:update(dt)
                if game_object.dead then 
                    game_object:destroy()
                    table.remove(self.game_objects, i) 
...
function Area:draw()
    ...
    local menu2, menu, ui, fore, main, back = {}, {}, {}, {}, {}, {}
    for _, game_object in ipairs(self.game_objects) do
        if game_object.layer == 'menu2' then
            table.insert(menu2, game_object)
        end
        if game_object.layer == 'menu' then
            table.insert(menu, game_object)
        end
        ...
        for _, game_object in ipairs(menu) do
            game_object:draw()
        end
        for _, game_object in ipairs(menu2) do
            game_object:draw()
        end
end
